...
def run_simulation(options, out_folder="out"):
    ...
    latest_time = subprocess.check_output("foamListTimes -latestTime", cwd="fluid-openfoam", shell=True, text=True).strip()
    subprocess.run(["mv", latest_time, f'output/T{latest_time}-{out_folder}'], cwd="fluid-openfoam", stdout=subprocess.DEVNULL)
...
if __name__ == "__main__":
    timesteps = [
        ...
        0.00005, 0.00004, 0.00002, 0.00001
    ]
    ...
    for dt in timesteps:
        ...
        options = {
            ...
            'endTime': 1,
            'maxTime': 1,
            'solidWriteInterval': round(1 / (n_samples * dt)),
            'fluidWriteInterval': 0.1
        }

        setup_simulation(options)

        out_folder = f'C0.9-E{options["endTime"]}-dt{options["timestep"]}'
